...
class TableModel:
    ...
    async def resgister_table(data : TableSchema):
        try:
            ...
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                await cursor.execute(
                    "INSERT INTO tables (table_number, capacity, available) VALUES (%s, %s, %s)",
                    (data.table_number, data.capacity, data.available)
                )
                await conn.commit()
    ...
    async def update_table(id: int | str, data : TableSchema):
        try:
            ...
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                await cursor.execute(
                    """UPDATE tables SET table_number = %s, capacity = %s, available = %s    
                        WHERE table_ID = %s
                    """, (data.table_number, data.capacity, data.available, id)
                )
