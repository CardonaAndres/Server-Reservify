...
class ReservationModel:
    @staticmethod
    async def get_reservation_paginate(user_ID : int | str, page : int = 1):
        try:
            conn = await db_connection()
            offset = (page - 1) * 10
            
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                query = '''SELECT r.*, t.table_number, t.capacity, t.available 
    ...
    @staticmethod
    async def get_reservation(user_ID : int, id : int | str):
        try:
            conn = await db_connection()
            ...
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                query = """SELECT r.*, t.table_number, t.capacity, t.available 
                           FROM reservations r INNER JOIN tables t ON t.table_ID = r.table_ID 
                           WHERE r.user_ID = %s AND r.reservation_ID = %s"""
                
                await cursor.execute(query, (user_ID, id))
