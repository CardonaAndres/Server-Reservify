...
class ReservationController:
    ...
    async def get_all_reservations(user : dict, page : int = 1):
        try:
            ...
            user_reservations = await ReservationModel.get_reservation_paginate(
                user['user_ID'], page
            )
    ...
    async def get_reservation(id : int | str, user : dict):
        try:
            ...
            reservation = await ReservationModel.get_reservation(user['user_ID'], id)
                ...
                if item['table_ID'] == data.table_ID:
                    ...
                    if (item['user_ID'] == user['user_ID'] and item['status'] not in 
                        ...
                        return JSONResponse(
                            ...
                            content = {
                                'message' : f'Esta mesa ya tiene una reserva {item['status']} a tu nombre, en la fecha y hora dada'
                            }
                        )
    ...
    async def update_reservation(id : int | str, data : ReservationSchema, user : dict):
        try:
            ...
            existing_reservation = await ReservationModel.get_reservation(data.user_ID, id)
            existing_table = await TableModel.get_table(data.table_ID)
    ...
    async def delete_reservation(id : int | str, user : dict):
        try:
            ...
            existing_reservation = await ReservationModel.get_reservation(user['user_ID'], id)
